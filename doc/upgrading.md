...
1. From the latest [Release](https://github.com/TheStaticMage/firebot-mage-trivia/releases), download `Firebot-MageTrivia-<version>.js`.
2. Place the new `Firebot-MageTrivia-<version>.js` into your Firebot scripts directory (File > Open Data Folder, then select the "scripts" directory).
3. Go to Settings > Scripts > Manage Startup Scripts, find the plugin in the list, click Edit next to it, select the new `Firebot-MageTrivia-<version>.js` file, and click Save.
4. Restart Firebot.
...
:fire: **Firebot 5.65 Requirement**: Version 0.1.0 and later require Firebot 5.65 or later. If you are using Firebot 5.64, you must use version 0.0.4.
