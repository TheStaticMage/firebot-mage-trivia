...
This event fires periodically while a question is active to acknowledge users that have answered the question.
...
Sets [`$mageTriviaAnswerAccepted`](/doc/reference/variables.md#magetriviaansweraccepted) to an **Array** of users who have answered the question recently the last event (or the start of the question).
...
- This event fires only while a question is active.
- The frequency of the event can be customized in the [settings](/doc/reference/settings.md#answer-confirmation-interval).
- The event also fires immediately before the question ends (before the [Question Ended](#question-ended) event), assuming of course that there is at least one answer to acknowledge.
...
- [YAML question file](/doc/questions.md) is not configured but the File source is selected
- [YAML question file](/doc/questions.md) could not be read or parsed
...
This event fires when a trivia question is successfully cancelled (in response to the [Cancel Trivia Question](/doc/reference/effects.md#cancel-trivia-question) effect).

#### Handling this Event

You might want to post a message in chat, such as `The trivia question has been cancelled.` Or you might not want to do anything. (Cancelling an in-progress question will be rare and only occurs as a direct result of your request to do so, and as such you are probably talking about it on your stream.)

Notes:

- You do _not_ need to make any currency adjustments in response to this event. The program handles all currency adjustments for trivia questions.
...
This event fires when a question has ended normally.

Note: This event does _not_ fire when a question is cancelled (the [Question Cancelled](#question-cancelled) event fires in that case) or if a question fails to start correctly (the [Critical Error](#critical-error) event fires in that case).
...
```
Trivia question ended. Correct answer: $arrayJoinWith[$mageTriviaCorrectAnswers]. $if[$arrayLength[$mageTriviaWinnersWithPoints] == 0, Nobody answered the question correctly, Congratulations to $arrayJoinWith[$mageTriviaWinnersWithPoints,and] for answering correctly]!
```
...
- You do _not_ need to make currency adjustments, such as awarding currency to winners or penalizing incorrect answers. The program handles all currency adjustments for trivia questions.
...
This event fires when a question has been created (in response to the [Create Trivia Question](/doc/reference/effects.md#create-trivia-question) effect).

Note: This event does _not_ fire if there was an error creating the trivia question. In that case, a [Critical Error](#critical-error) event is fired instead.

#### Implementation

You will generally want to provide the question and answer choices to the players, possibly along with instructions to play.
...
There is definitely room for creativity. For example, I use a browser source to [display the trivia questions and answers on my stream](/doc/customization.md#displaying-the-question-on-screen) in addition to chat.
...
- Called the "Cancel Trivia Question" effect when there was no trivia question in progress. (The call will be ignored.)
- Called the "Create Trivia Question" effect when there was already a trivia question in progress. (The call will be ignored.)
